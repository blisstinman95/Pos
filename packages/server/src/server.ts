...
import { z } from "zod";
import { productSchema, type Product } from "@pos/shared";
...
// Stock endpoints
app.get("/api/stock/levels", (_req: Request, res: Response) => {
...
app.post("/api/stock/movements", (req: Request, res: Response) => {
  ...
  const m = parsed.data;
  db.prepare(
    `INSERT OR REPLACE INTO stock_movements (id, product_id, type, quantity, reason, reference, created_at, user_id)
     VALUES (@id, @productId, @type, @quantity, @reason, @reference, @createdAt, @userId)`
  ).run(m);

  // Update stock level
  const current = db.prepare("SELECT * FROM stock_levels WHERE product_id = ?").get(m.productId) as any;
  const newQty = (current?.quantity ?? 0) + (m.type === "in" ? m.quantity : m.type === "out" ? -m.quantity : m.quantity);
  const now = new Date().toISOString();
  if (!current) {
    db.prepare(
      `INSERT INTO stock_levels (id, product_id, quantity, location, updated_at)
       VALUES (?, ?, ?, 'main', ?)`
    ).run(m.id, m.productId, newQty, now);
  } else {
...
app.post("/api/sales", (req: Request, res: Response) => {
  ...
  const tx = db.transaction(() => {
    db.prepare(
      `INSERT OR REPLACE INTO sales (id, number, created_at, user_id, customer_id, total_amount, currency, payment_method)
       VALUES (@id, @number, @createdAt, @userId, @customerId, @totalAmount, @currency, @paymentMethod)`
    ).run(sale);

    const insertLine = db.prepare(
      `INSERT OR REPLACE INTO sale_lines (id, sale_id, product_id, quantity, unit_price, line_total)
    ...
    for (const line of lines) {
      insertLine.run(line);
      // decrease stock
      const current = db.prepare("SELECT * FROM stock_levels WHERE product_id = ?").get(line.productId) as any;
      const now = new Date().toISOString();
      ...
      if (!current) {
        db.prepare(
          `INSERT INTO stock_levels (id, product_id, quantity, location, updated_at)
           VALUES (?, ?, ?, 'main', ?)`
        ).run(line.id, line.productId, newQty, now);
      } else {
        db.prepare("UPDATE stock_levels SET quantity = ?, updated_at = ? WHERE id = ?").run(newQty, now, current.id);
      }
    }
...
app.post("/api/sync", (req: Request, res: Response) => {
  ...
  const upsertMany = (table: string, rows: any[]) => {
  ...
  const pull = (table: string, mapper: (row: any) => any) => {
...
function mapStockLevelToDb(s: any) {
  return {
    id: s.id,
...
function mapStockLevelFromDb(row: any) {
  return {
    id: row.id,
...
function mapStockMovementToDb(m: any) {
  return {
    id: m.id,
...
function mapSaleLineToDb(l: any) {
  return {
    id: l.id,
    sale_id: l.saleId,
